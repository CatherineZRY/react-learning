...
import { addCartList, reduceCartList } from '../../../store/modules/menuListStore';
...
const Foods = ({
  ...
  const onPlus = (curFood) => {
    return () => dispatch(addCartList(curFood));
  };
  const onMinus = (curFood) => {
    return () => dispatch(reduceCartList(curFood));
  };
      ...
      <div className="goods-info">
        ...
        <div className="goods-price-count">
          ...
          <div className="goods-count">
            {
              count ?
                <Count count={count} onPlus={onPlus(curFood)} onMinus={onMinus(curFood)} /> : <span className="plus" onClick={() => dispatch(addCartList(curFood))}></span>
            }
          </div>
        </div>
      </div>
